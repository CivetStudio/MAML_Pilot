    ...
    for root, dirs, files in os.walk(input_folder):
        for filename in files:
            ...
            if filename.endswith(('.png', '.jpg', '.jpeg', '.bmp', '.gif', '.webp')):
                ...
                delete_repeat = 0
                if delete_repeat:
                    # _split_ = os.path.basename(filename)[:5]
                    # _split_ = os.path.basename(filename)[:7]
                    # file_index = filename.split(_split_)[1].split('.')[0]

                    file_index = filename.split('.')[0][-4:]
                    # file_index = 100
                    # print(file_index)
                    # if int(file_index) % 2 == 0:
                    if int(file_index) % 4 != 0:
                        print(file_index)
                ...
                if _pass:
                    ...
                    #     resize = 1
                    resize = 0
                    if resize and 'img_' in filename:
